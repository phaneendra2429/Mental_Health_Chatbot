...
conv_chain_with_memory = ConversationChain(llm=llm,
                                  memory=memory,
                                 verbose=True)



"""def ConvoMemoryLLM(chat_history, query):   
    prompt2 = PromptTemplate(template=template, input_variables=['chat_history','question'])
    template = "Act as a therapist, and conduct therapy sessions with the user. Your goal is to strictly only ask one question to analyse their mental health problem, after understanding the following input:{query}.Do not generate text related to the input."
    response= conv_chain_with_memory.run(query)
    return response"""
